...
export function test_editions$ (
  ...
  let creator_addr, owner0_addr, owner1_addr, token_id;
  token_id = create_token$(creator, u64("2"), $c);
  creator_addr = Std.Signer.address_of$(creator, $c);
  owner0_addr = Std.Signer.address_of$(owner0, $c);
  owner1_addr = Std.Signer.address_of$(owner1, $c);
  offer$(creator, $.copy(owner0_addr), $.copy(token_id), u64("1"), $c);
  if (!Table.length$($c.borrow_global<TokenTransfers>(new StructTag(new HexString("0x1"), "TokenTransfers", "TokenTransfers", []), $.copy(creator_addr)).pending_claims, $c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "Table", "Table", [new StructTag(new HexString("0x1"), "Token", "TokenId", []), new StructTag(new HexString("0x1"), "Token", "Token", [])])] as TypeTag[]).eq(u64("1"))) {
    throw $.abortCode(u64("1"));
  }
  offer$(creator, $.copy(owner1_addr), $.copy(token_id), u64("1"), $c);
  if (!Table.length$($c.borrow_global<TokenTransfers>(new StructTag(new HexString("0x1"), "TokenTransfers", "TokenTransfers", []), $.copy(creator_addr)).pending_claims, $c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "Table", "Table", [new StructTag(new HexString("0x1"), "Token", "TokenId", []), new StructTag(new HexString("0x1"), "Token", "Token", [])])] as TypeTag[]).eq(u64("2"))) {
    throw $.abortCode(u64("2"));
  }
  claim$(owner0, $.copy(creator_addr), $.copy(token_id), $c);
  if (!Table.length$($c.borrow_global<TokenTransfers>(new StructTag(new HexString("0x1"), "TokenTransfers", "TokenTransfers", []), $.copy(creator_addr)).pending_claims, $c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "Table", "Table", [new StructTag(new HexString("0x1"), "Token", "TokenId", []), new StructTag(new HexString("0x1"), "Token", "Token", [])])] as TypeTag[]).eq(u64("1"))) {
    throw $.abortCode(u64("3"));
  }
  claim$(owner1, $.copy(creator_addr), $.copy(token_id), $c);
  if (!Table.length$($c.borrow_global<TokenTransfers>(new StructTag(new HexString("0x1"), "TokenTransfers", "TokenTransfers", []), $.copy(creator_addr)).pending_claims, $c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "Table", "Table", [new StructTag(new HexString("0x1"), "Token", "TokenId", []), new StructTag(new HexString("0x1"), "Token", "Token", [])])] as TypeTag[]).eq(u64("0"))) {
    throw $.abortCode(u64("4"));
  }
  offer$(owner0, $.copy(owner1_addr), $.copy(token_id), u64("1"), $c);
  claim$(owner1, $.copy(owner0_addr), $.copy(token_id), $c);
  offer$(owner1, $.copy(creator_addr), $.copy(token_id), u64("1"), $c);
  offer$(owner1, $.copy(creator_addr), $.copy(token_id), u64("1"), $c);
  claim$(creator, $.copy(owner1_addr), $.copy(token_id), $c);
  return;
}
...
export function test_nft$ (
  ...
  let creator_addr, owner_addr, token_id;
  token_id = create_token$(creator, u64("1"), $c);
  creator_addr = Std.Signer.address_of$(creator, $c);
  owner_addr = Std.Signer.address_of$(owner, $c);
  offer$(creator, $.copy(owner_addr), $.copy(token_id), u64("1"), $c);
  claim$(owner, $.copy(creator_addr), $.copy(token_id), $c);
  offer$(owner, $.copy(creator_addr), $.copy(token_id), u64("1"), $c);
  cancel_offer$(owner, $.copy(creator_addr), $.copy(token_id), $c);
  return;
}
