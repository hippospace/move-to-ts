...
export class TestProposal 
{
  static moduleAddress = moduleAddress;
  static moduleName = moduleName;
  static structName: string = "TestProposal";
  static typeParameters: TypeParamDeclType[] = [
  ...
  static TestProposalParser(data:any, typeTag: TypeTag, repo: AptosParserRepo) : TestProposal {
    const proto = $.parseStructProto(data, typeTag, repo, TestProposal);
    return new TestProposal(proto, typeTag);
  }
...
export function create_test_proposal$ (
  ...
  let governance_address, proposal, proposal_id;
  register$(governance, $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]);
  governance_address = Std.Signer.address_of$(governance, $c);
  proposal = new TestProposal({  }, new StructTag(new HexString("0x1"), "Voting", "TestProposal", []));
  proposal_id = create_proposal$($.copy(governance_address), proposal, [], u128("10"), u64("100000"), $.copy(early_resolution_threshold), $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]);
  return $.copy(proposal_id);
}
...
export function test_voting_failed$ (
  ...
  let governance_address, proof, proposal, proposal_id;
  Timestamp.set_time_has_started_for_testing$(core_resources, $c);
  governance_address = Std.Signer.address_of$(governance, $c);
  proposal_id = create_test_proposal$(governance, Std.Option.none$($c, [AtomicTypeTag.U128] as TypeTag[]), $c);
  proof = new TestProposal({  }, new StructTag(new HexString("0x1"), "Voting", "TestProposal", []));
  vote$(proof, $.copy(governance_address), $.copy(proposal_id), u64("10"), true, $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]);
  vote$(proof, $.copy(governance_address), $.copy(proposal_id), u64("100"), false, $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]);
  proof;
  Timestamp.update_global_time_for_test$(u64("100001000000"), $c);
  if (!get_proposal_state$($.copy(governance_address), $.copy(proposal_id), $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]).eq(PROPOSAL_STATE_FAILED)) {
    throw $.abortCode(u64("1"));
  }
  proposal = resolve$($.copy(governance_address), $.copy(proposal_id), $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]);
  proposal;
  return;
}
...
export function test_voting_failed_early$ (
  ...
  Timestamp.set_time_has_started_for_testing$(core_resources, $c);
  governance_address = Std.Signer.address_of$(governance, $c);
  proposal_id = create_test_proposal$(governance, Std.Option.some$(u128("100"), $c, [AtomicTypeTag.U128] as TypeTag[]), $c);
  proof = new TestProposal({  }, new StructTag(new HexString("0x1"), "Voting", "TestProposal", []));
  vote$(proof, $.copy(governance_address), $.copy(proposal_id), u64("100"), true, $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]);
  vote$(proof, $.copy(governance_address), $.copy(proposal_id), u64("100"), false, $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]);
  proof;
  Timestamp.update_global_time_for_test$(u64("100001000000"), $c);
  if (!get_proposal_state$($.copy(governance_address), $.copy(proposal_id), $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]).eq(PROPOSAL_STATE_FAILED)) {
...
export function buildPayload_test_voting_failed_early (
  ...
  const typeParamStrings = [] as string[];
  return $.buildPayload(
    "0x1::Voting::test_voting_failed_early",
    typeParamStrings,
    []
  );
...
export function test_voting_passed$ (
  ...
  let governance_address, proof, proposal, proposal_id, voting_forum;
  Timestamp.set_time_has_started_for_testing$(core_resources, $c);
  governance_address = Std.Signer.address_of$(governance, $c);
  proposal_id = create_test_proposal$(governance, Std.Option.none$($c, [AtomicTypeTag.U128] as TypeTag[]), $c);
  if (!get_proposal_state$($.copy(governance_address), $.copy(proposal_id), $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]).eq(PROPOSAL_STATE_PENDING)) {
  ...
  proof;
  Timestamp.update_global_time_for_test$(u64("100001000000"), $c);
  if (!get_proposal_state$($.copy(governance_address), $.copy(proposal_id), $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]).eq(PROPOSAL_STATE_SUCCEEDED)) {
    throw $.abortCode(u64("1"));
  }
  proposal = resolve$($.copy(governance_address), $.copy(proposal_id), $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]);
  voting_forum = $c.borrow_global<VotingForum>(new StructTag(new HexString("0x1"), "Voting", "VotingForum", [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])]), $.copy(governance_address));
  if (!!Table.contains$(voting_forum.proposals, $.copy(proposal_id), $c, [AtomicTypeTag.U64, new StructTag(new HexString("0x1"), "Voting", "Proposal", [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])])] as TypeTag[])) {
...
export function test_voting_passed_early$ (
  ...
  proof = new TestProposal({  }, new StructTag(new HexString("0x1"), "Voting", "TestProposal", []));
  vote$(proof, $.copy(governance_address), $.copy(proposal_id), u64("100"), true, $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]);
  vote$(proof, $.copy(governance_address), $.copy(proposal_id), u64("10"), false, $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]);
  proof;
  if (!get_proposal_state$($.copy(governance_address), $.copy(proposal_id), $c, [new StructTag(new HexString("0x1"), "Voting", "TestProposal", [])] as TypeTag[]).eq(PROPOSAL_STATE_SUCCEEDED)) {
    throw $.abortCode(u64("1"));
  }
...
export function buildPayload_test_voting_passed_early (
  ...
  const typeParamStrings = [] as string[];
  return $.buildPayload(
    "0x1::Voting::test_voting_passed_early",
    typeParamStrings,
    []
  );
