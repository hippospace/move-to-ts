...
export function end_to_end$ (
  ...
  let _resource_cap, bytes, container, resource, resource_addr, resource_cap, seed, user_addr;
  user_addr = Std.Signer.address_of$(user, $c);
  Account.create_account$($.copy(user_addr), $c);
  seed = [u8("1")];
  bytes = Std.BCS.to_bytes$(user_addr, $c, [AtomicTypeTag.Address] as TypeTag[]);
  Std.Vector.append$(bytes, $.copy(seed), $c, [AtomicTypeTag.U8] as TypeTag[]);
  resource_addr = Account.create_address_for_test$(Std.Hash.sha3_256$($.copy(bytes), $c), $c);
  create_resource_account$(user, $.copy(seed), Std.Vector.empty$($c, [AtomicTypeTag.U8] as TypeTag[]), $c);
  container = $c.borrow_global<Container>(new StructTag(new HexString("0x1"), "ResourceAccount", "Container", []), $.copy(user_addr));
  resource_cap = SimpleMap.borrow$(container.store, resource_addr, $c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "Account", "SignerCapability", [])] as TypeTag[]);
  resource = Account.create_signer_with_capability$(resource_cap, $c);
  _resource_cap = retrieve_resource_account_cap$(resource, $.copy(user_addr), $c);
  return;
}
...
export function buildPayload_end_to_end (
  ...
  const typeParamStrings = [] as string[];
  return $.buildPayload(
    "0x1::ResourceAccount::end_to_end",
    typeParamStrings,
    []
  );
