...
export function hash_map_bucket_index_test$ (
  ...
  let i, index, j, map;
  map = new__$(u64("8"), $c, [AtomicTypeTag.U64, AtomicTypeTag.U64] as TypeTag[]);
  if (!$.copy(map.level).eq(u8("3"))) {
    throw $.abortCode(u64("0"));
  }
  i = u64("0");
  while ($.copy(i).lt(u64("4"))) {
    {
      split_one_bucket$(map, $c, [AtomicTypeTag.U64, AtomicTypeTag.U64] as TypeTag[]);
      i = $.copy(i).add(u64("1"));
    }
  ...
  if (!$.copy(map.num_buckets).eq(u64("12"))) {
    throw $.abortCode(u64("0"));
  }
  i = u64("0");
  while ($.copy(i).lt(u64("256"))) {
    {
      j = $.copy(i).and(u64("15"));
      if ($.copy(j).ge($.copy(map.num_buckets))) {
        j = $.copy(j).xor(u64("8"));
      }
      else{
      }
      index = bucket_index$($.copy(map.level), $.copy(map.num_buckets), $.copy(i), $c);
      if (!$.copy(index).eq($.copy(j))) {
        throw $.abortCode(u64("0"));
      }
      i = $.copy(i).add(u64("1"));
    }
...
export function hash_map_split_test$ (
  ...
  let i, level, map;
  map = new__$(u64("1"), $c, [AtomicTypeTag.U64, AtomicTypeTag.U64] as TypeTag[]);
  i = u64("1");
  level = u8("0");
  while ($.copy(i).le(u64("256"))) {
    {
      if (!$.copy(map.num_buckets).eq($.copy(i))) {
        throw $.abortCode(u64("0"));
      }
      if (!$.copy(map.level).eq($.copy(level))) {
        throw $.abortCode($.copy(i));
      }
      split_one_bucket$(map, $c, [AtomicTypeTag.U64, AtomicTypeTag.U64] as TypeTag[]);
      i = $.copy(i).add(u64("1"));
      if ($.copy(i).eq(u64("1").shl($.copy(level).add(u8("1"))))) {
        level = $.copy(level).add(u8("1"));
      }
      else{
      }
    }
...
export function hash_map_test$ (
  ...
  map = new__$(u64("1"), $c, [AtomicTypeTag.U64, AtomicTypeTag.U64] as TypeTag[]);
  i = u64("0");
  while ($.copy(i).lt(u64("200"))) {
    {
      add$(map, $.copy(i), $.copy(i), $c, [AtomicTypeTag.U64, AtomicTypeTag.U64] as TypeTag[]);
      i = $.copy(i).add(u64("1"));
    }

  }if (!length$(map, $c, [AtomicTypeTag.U64, AtomicTypeTag.U64] as TypeTag[]).eq(u64("200"))) {
    throw $.abortCode(u64("0"));
  }
  i = u64("0");
  while ($.copy(i).lt(u64("200"))) {
    {
      $.set(borrow_mut$(map, $.copy(i), $c, [AtomicTypeTag.U64, AtomicTypeTag.U64] as TypeTag[]), $.copy(i).mul(u64("2")));
      if (!$.copy(borrow$(map, $.copy(i), $c, [AtomicTypeTag.U64, AtomicTypeTag.U64] as TypeTag[])).eq($.copy(i).mul(u64("2")))) {
        throw $.abortCode(u64("0"));
      }
      i = $.copy(i).add(u64("1"));
    }
  ...
  if (!$.copy(map.num_buckets).gt(u64("20"))) {
    throw $.abortCode($.copy(map.num_buckets));
  }
  while ($.copy(i).lt(u64("200"))) {
    {
      if (!contains$(map, i, $c, [AtomicTypeTag.U64, AtomicTypeTag.U64] as TypeTag[])) {
        throw $.abortCode(u64("0"));
      }
      if (!remove$(map, i, $c, [AtomicTypeTag.U64, AtomicTypeTag.U64] as TypeTag[]).eq($.copy(i).mul(u64("2")))) {
        throw $.abortCode(u64("0"));
      }
      i = $.copy(i).add(u64("1"));
    }
