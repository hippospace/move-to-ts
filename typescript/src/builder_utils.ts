...
export class TypeTagParser {
  ...
  private parseCommaList(endToken: TokenValue, allowTraillingComma: boolean): TxnBuilderTypes.TypeTag[] {
...
export function serializeArg(argVal: any, argType: TxnBuilderTypes.TypeTag, serializer: BCS.Serializer) {
  ...
  if (argType instanceof TypeTagStruct) {
    const { address, module_name: moduleName, name } = (argType as TxnBuilderTypes.TypeTagStruct).value;
    if (
